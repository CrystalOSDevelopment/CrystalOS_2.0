...
namespace CrystalOS2.Graphic_engines._3d_render
{
    public static class rendering
    {
        public static Canvas vbe;
        ...
        public static void trapez(int x, int y, int width, int height, int side)
        {
            while(pointer < width)
            {
                if(prev_pointer < side)
                {
                    ...
                    {
                        prev_pointer++;
                        //ImprovedVBE.DrawLine(0, x + pointer, y - prev_pointer, height - prev_pointer * 2);
                    }
                    else
                    {
                        //ImprovedVBE.DrawLine(0, x + pointer, y - prev_pointer, height - prev_pointer * 2);
            ...
            while (pointer > 1)
            {
                if (prev_pointer <= side)
                {
                    ...
                    {
                        prev_pointer--;
                        //ImprovedVBE.DrawLine(0, x - pointer + 1, y - prev_pointer, height - prev_pointer * 2);
                    }
                    else
                    {
                        //ImprovedVBE.DrawLine(0, x - pointer + 1, y - prev_pointer, height - prev_pointer * 2);
